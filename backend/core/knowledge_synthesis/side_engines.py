...
class UncertaintyBudgetEngine:
    ...
    def run(self, business_text: str, answers: dict[str, str] | None = None) -> UncertaintyBudget:
        ...
        markers = {
            "who_pays": ("клиент", "buyer", "b2b", "b2c", "плат", "customer"),
            "unit_of_value": ("тонн", "заказ", "unit", "sku", "час", "подписк"),
            "first_channel": ("telegram", "whatsapp", "x.com", "сайт", "маркетплейс", "холод"),
            "constraint_cash": ("бюджет", "cash", "капитал", "без денег", "bootstrap"),
            "constraint_time": ("дней", "недел", "месяц", "asap", "срок"),
            "geography": ("город", "регион", "rf", "eu", "local", "москв", "online"),
            "regulatory": ("лиценз", "отход", "ндс", "compliance", "закон"),
            "success_metric": ("метрик", "kpi", "марж", "маржа", "conversion", "roi"),
